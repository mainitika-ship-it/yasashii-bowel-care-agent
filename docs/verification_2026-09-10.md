...
## One-command local setup follow-up

`python3 src/check_local_setup.py` now discovers existing Ollama models, checks local/tool metadata and pinned Python package versions, and prints safely quoted next commands when ready. It makes metadata requests only, checks at most eight model names, ignores proxy settings, and refuses redirects. It never runs inference, installs packages, downloads weights, writes files, or changes providers. "確認必要" means an unmet or unverified setup condition; it is not a care event or a medical judgment.

**108 tests passed locally.** Additional checks cover metadata-only requests, malformed lists, unsafe model names, bounded discovery, paths containing spaces, redacted connection errors, and preventing a suggested live command when packages are missing. A standard-library-only CLI run in the build environment correctly reported that Ollama was unavailable and exited with status 1. This is expected failure-path evidence, not a test of the owner's Mac or an actual model. The next owner-side action is the single setup command above; public video and final submission remain outstanding.
